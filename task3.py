...
def vowels(word):
    schedule = "АЕЁИОУЭЮЯаеёиоуэюя"
    return sum(1 for letter in word if letter in schedule)

def rhythm(text):
    phrases = text.split()
    count = list(map(lambda phrase: vowels(phrase.replace('-', '')), phrases))


    if all(count[0] == x for x in count):
        return "Парам пам-пам"
    else:
        return "Пам парам"

text = input("Введите текст фразы: ")
